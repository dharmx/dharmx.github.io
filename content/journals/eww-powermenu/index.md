...
lastmod: 2022-08-21T00:09:26+05:30
draft: false
...
cover:
  image: /journals/eww-powermenu/imgs/powermenu.png
  alt: powermenu
  caption: A classy powermenu using Eww

editPost:
  URL: "https://github.com/dharmx/dharmx.github.io/tree/main/content"
...
Note that, this has a very steep learning curve. Which is because tutorials,
demo and examples about this are quite scarce regarding this software. And
the examples are either too complex or, very poorly documented. Most of them
do not have any form of documentation.
...
> **Tip**: You may use [command-not-found](https://command-not-found.com) if
  you do not know what the package
  is called for your distro.
...
- Fonts used in this widget are Poiret One, Koulen, Phosphor.
...
cd eww
# then install the *-devel variants of you distro
...
> **Tip**: It is recommended to install [DSL](https://en.wikipedia.org/wiki/Domain-specific_language)
...
> **Tip**: For Vim users you might want to use [vim-parinfer](https://github.com/bhurlow/vim-parinfer)
  for auto-indentation. Note that, the author of [yuck.vim](https://github.com/elkowar/yuck.vim)
  has already implemented auto-indentation to a specific degree. Lastly, since I
  am a [Neovim](https://neovim.io) user therefore, I am obligated to say this;
  You might want to use [nvim-parinfer](https://github.com/gpanders/nvim-parinfer)
  by [gpanders](https://github.com/gpanders) instead of the Vim version as this
  one is written in pure Lua which is faster than the VimL version. Although this
  one is a bit underdeveloped.
...
house one widget. Now, a said widget may be a `box` which has the ability to house
multiple widgets &#8212; a `label` which will display a text. More on this later.
...
> **Note**: Always use `%` for widths and heights. DO NOT hardcode dimensions.
...
This is a widget that will display the string "Hello, World!" once it is hooked
into a window as you have seen above.
...
But, I would advise you not to do that since, it looks quite congested and hard-to-read.
...
Look at the cover photo of this article. Notice that there are Network, Time
and Battery information. Those are not interactive widgets but they dynamic.
Or, rather they display dynamic data. Like, lets take the Time label at the
bottom left corner. This label will display the new time value whenever the
time changes.

So, if the time becomes `12:00` to `12:01`, the label will reflect that.
This is NOT automatic. You need to configure this behavior using a poll
variable, that will run the `date` command by a specific interval and capture
the `STDOUT`.
...
Since it is a well known language I won't be teaching you Scss/CSS. Because
quite frankly, there are numerous sites and vidoes that will explain this better.
...
You may also want to learn CSS first if you do not know it.
...
This may seem overwhelming... because you might be thinking you need to learn
and digest all of the above links. This is just plain wrong. Just get a general
idea of CSS (yes I did not say Scss) and then use all above links as mere
references. Use them as [Google](https://youtu.be/avCWDDox1nE) i.e., refer to
the documentation, tutorial and videos _only_ if you are stuck or, want to
...
Where was I again? Ah! Right. This section will be divided into 5 subsections.
...
See the running-the-powermenu section to view the Eww window.

> **Note**: You need to replace `(powermenu_layout)` with "Testing..."
...
Now, to incorporate these into variables.
...
This section is divided into five specific sections.
...
These options or, properties can be applied to ant built-in widgets. Like
...
{{< figure
  ...
  caption="Evidently, the result will be a tug o' war between the labels and then one with the larger content length will win!"
>}}
...
`shutdown_icon`, `logout_icon` as function parameters.
...
okay, slow and offline. This will be done using [ternary]() operators. To
illustrate this better following is a comparison between ternary operators
and normal conditionals in C.
...
Battery current capacity is classified by eight levels. Namely, `one`,
`two`, `three`, `four`, `five`, `six`, `seven` and `charge`.

![battery](./imgs/battery.png)
...
> **Note**: The glyphs may or, may not be visible as, you might not have the
  font installed.
...
> **Note**: `:class "name"` will correspond to `.name { color: pink; }` in Scss.
...
![blog powermenu showcase](./imgs/showcase.png)

There are many neat `eww` flags that might interest you. Try `eww --help`
for starters. If it does not 'work' then try looking at the Debugging section.
Debugging section might also help you in solving general problems related to
Eww.

## Debugging

Following are some of the things that you _should_ do before anything else if
the widget seems buggy or, not reflecting the changes that you made.
...
- If you notice that there are predefined colors, paddings, margins,
  etc. Then add this `* { all: unset }` at the top of your Scss file
  will disable that.
- GTK Inspector is probably the most useful tool for debugging not just Eww widgets
  ...
  ![eww inspector](./imgs/inspector.png)

## Tips

- If you having difficulties implementing your own widget(s) then it might be a
...
- I initally planned to do this in this one but, decided not to as it would be too
...
![showcase light](./imgs/light.png)

![showcase dark](./imgs/powermenu.png)
...
- <https://github.com/dharmx/vile/tree/main/src/yuck/scss>
- <https://github.com/Vermoot/dotfiles>

## Ending Note

If you have any questions, do not hesitate to ask them in the comments
section! (Oh wait. You can't 🤓).
And I hope you enjoyed building this as much I hated writing this. Cheers 🍻.
